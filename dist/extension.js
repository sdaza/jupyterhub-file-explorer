...
const FileExplorer_1 = __webpack_require__(2);
// Module-level variables for cleanup
let fileExplorerProvider;
let reconnectTimer;
let reconnectAttempts = 0;
let lastConnection;
let healthCheckInterval;
function activate(context) {
    fileExplorerProvider = new FileExplorer_1.FileExplorerProvider();
    const jupyterContentProvider = new FileExplorer_1.JupyterContentProvider(fileExplorerProvider);
    ...
    const startHealthChecks = () => {
        ...
        const healthCheckIntervalMs = config.get('healthCheckInterval', 30000); // 30 seconds
        if (enableHealthChecks && !healthCheckInterval) {
            healthCheckInterval = setInterval(() => __awaiter(this, void 0, void 0, function* () {
                if (fileExplorerProvider && lastConnection) {
                    const isHealthy = yield fileExplorerProvider.checkConnectionHealth();
                    if (!isHealthy) {
    ...
    // Start health checks
    startHealthChecks();
    const connectToJupyter = (connection) => __awaiter(this, void 0, void 0, function* () {
        try {
            yield fileExplorerProvider.setConnection(connection.url, connection.token, connection.remotePath || '/', connection.name);
            ...
            if (axiosInstance) {
                ...
                const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
                yield config.update('lastConnection', connection, vscode.ConfigurationTarget.Global);
            ...
            if (autoReconnect && lastConnection && reconnectAttempts < maxAttempts) {
                ...
                const baseInterval = config.get('reconnectInterval', 5000);
                const backoffInterval = baseInterval * Math.pow(2, reconnectAttempts - 1);
                vscode.window.showWarningMessage(`Connection lost. Attempting to reconnect in ${backoffInterval / 1000}s (${reconnectAttempts}/${maxAttempts})...`);
    ...
    let connectDisposable = vscode.commands.registerCommand('jupyterFileExplorer.connectJupyter', () => __awaiter(this, void 0, void 0, function* () {
        const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
        ...
        if (connections.length === 0) {
            ...
            yield vscode.commands.executeCommand('jupyterFileExplorer.addJupyterConnection');
            return;
        }
        const selected = yield vscode.window.showQuickPick(connections.map(c => c.name), {
            placeHolder: 'Select a Jupyter Server to connect to'
        });
        if (selected) {
            const connection = connections.find(c => c.name === selected);
            if (connection) {
                yield connectToJupyter(connection);
            }
        }
    }));
    let addConnectionDisposable = vscode.commands.registerCommand('jupyterFileExplorer.addJupyterConnection', () => __awaiter(this, void 0, void 0, function* () {
        const name = yield vscode.window.showInputBox({ prompt: 'Enter a name for this connection' });
        if (!name)
            return;
        const url = yield vscode.window.showInputBox({ prompt: 'Enter Jupyter Server URL', ignoreFocusOut: true });
        if (!url)
            return;
        const token = yield vscode.window.showInputBox({ prompt: 'Enter Jupyter Token', password: true, ignoreFocusOut: true });
        if (!token)
            return;
        const remotePath = yield vscode.window.showInputBox({ prompt: 'Enter Remote Path', value: '/', ignoreFocusOut: true });
        const newConnection = { name, url, token, remotePath: remotePath || '/' };
        const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
        const connections = config.get('connections') || [];
        connections.push(newConnection);
        yield config.update('connections', connections, vscode.ConfigurationTarget.Global);
        vscode.window.showInformationMessage(`Connection '${name}' saved.`);
    }));
    let selectConnectionDisposable = vscode.commands.registerCommand('jupyterFileExplorer.selectJupyterConnection', () => __awaiter(this, void 0, void 0, function* () {
        const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
        const connections = config.get('connections') || [];
        if (connections.length === 0) {
    ...
    let removeConnectionDisposable = vscode.commands.registerCommand('jupyterFileExplorer.removeJupyterConnection', () => __awaiter(this, void 0, void 0, function* () {
        const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
        const connections = config.get('connections') || [];
        if (connections.length === 0) {
        ...
        if (selected) {
            ...
            yield config.update('connections', updatedConnections, vscode.ConfigurationTarget.Global);
    ...
    let newFileDisposable = vscode.commands.registerCommand('jupyterFileExplorer.newFile', (item) => {
        fileExplorerProvider.newFile(item);
    });
    let newFolderDisposable = vscode.commands.registerCommand('jupyterFileExplorer.newFolder', (item) => {
    ...
    let uploadFileDisposable = vscode.commands.registerCommand('jupyterFileExplorer.uploadFile', (item) => {
        fileExplorerProvider.uploadFile(item);
    });
    let uploadFolderDisposable = vscode.commands.registerCommand('jupyterFileExplorer.uploadFolder', (item) => {
        fileExplorerProvider.uploadFolder(item);
    });
    let downloadFileDisposable = vscode.commands.registerCommand('jupyterFileExplorer.downloadFile', (item) => {
        fileExplorerProvider.downloadFile(item);
    });
    let renameFileDisposable = vscode.commands.registerCommand('jupyterFileExplorer.renameFile', (item) => {
        fileExplorerProvider.renameFile(item);
    });
    let deleteFileDisposable = vscode.commands.registerCommand('jupyterFileExplorer.deleteFile', (item) => {
        fileExplorerProvider.deleteFile(item);
    });
    let forceDeleteFileDisposable = vscode.commands.registerCommand('jupyterFileExplorer.forceDeleteFile', (item) => {
        fileExplorerProvider.forceDeleteFile(item);
    });
    let openFileDisposable = vscode.commands.registerCommand('jupyterFileExplorer.openFile', (filePath) => {
    ...
    context.subscriptions.push(connectDisposable, addConnectionDisposable, selectConnectionDisposable, removeConnectionDisposable, disconnectDisposable, refreshDisposable, newFileDisposable, newFolderDisposable, newFileInRootDisposable, newFolderInRootDisposable, uploadFileDisposable, uploadFolderDisposable, downloadFileDisposable, renameFileDisposable, deleteFileDisposable, forceDeleteFileDisposable, openFileDisposable);
    ...
    const autoConnectOnStartup = () => __awaiter(this, void 0, void 0, function* () {
        const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
        const autoConnect = config.get('autoConnect', false);
        if (autoConnect) {
            const savedLastConnection = config.get('lastConnection');
            if (savedLastConnection && savedLastConnection.name && savedLastConnection.url && savedLastConnection.token) {
                ...
                yield connectToJupyter(savedLastConnection);
            }
        }
    });
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.JupyterContentProvider = exports.FileItem = exports.FileExplorerProvider = void 0;
const vscode = __webpack_require__(1);
...
class FileExplorerProvider {
    constructor() {
        ...
        this._onConnectionLost = new vscode.EventEmitter();
        this.onConnectionLost = this._onConnectionLost.event;
        // Drag and drop support
        this.dropMimeTypes = [
            'text/uri-list',
            'application/vnd.code.tree.jupyterFileExplorer'
        ];
        this.dragMimeTypes = ['application/vnd.code.tree.jupyterFileExplorer'];
        this.jupyterServerUrl = '';
        this.jupyterToken = '';
        this.remotePath = '/';
        ...
        this.requestDelay = 100; // Minimum 100ms between requests
        this.cache = new Map();
    ...
    updateConfigSettings() {
        const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
        this.requestDelay = config.get('requestDelay', 100);
        this.cacheTimeout = config.get('cacheTimeout', 5000);
        // Clear cache if settings changed significantly
        const maxCacheSize = config.get('maxCacheSize', 100);
    ...
    setConnection(url, token, remotePath, connectionName) {
    ...
    getFileInfo(filePath) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const response = yield this.makeRequestWithCache(`/api/contents${filePath}`);
    ...
    checkConnectionHealth() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const response = yield this.axiosInstance.get('api/status', { timeout: 5000 });
    ...
    handleApiError(error, operation, showUserMessage = true) {
        var _a;
        ...
        if (axios_1.default.isAxiosError(error)) {
            if (error.response) {
                errorMessage += `: ${error.response.status} - ${((_a = error.response.data) === null || _a === void 0 ? void 0 : _a.message) || error.response.statusText}`;
            }
    ...
    adaptiveRateLimit(error) {
        var _a;
        if (error && (((_a = error.response) === null || _a === void 0 ? void 0 : _a.status) === 429 || error.code === 'ECONNABORTED')) {
    ...
    makeRequestWithCache(url, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const config = vscode.workspace.getConfiguration('jupyterFileExplorer');
            const enableCaching = config.get('enableCaching', true);
            ...
            if (method === 'GET' && enableCaching) {
                ...
                if (this.pendingRequests.has(pendingKey)) {
                    ...
                    try {
                        return yield this.pendingRequests.get(pendingKey);
                    }
                    catch (error) {
            ...
            try {
                const requestPromise = method === 'GET'
                    ...
                    : this.axiosInstance.request(Object.assign({ url }, options));
                if (method === 'GET' && enableCaching) {
    ...
    sortItems(items) {
        ...
        const sortBy = config.get('sortFiles', 'type');
        const sortOrder = config.get('sortOrder', 'asc');
        ...
        const comparators = {
            ...
            name: (a, b) => a.label.toLowerCase().localeCompare(b.label.toLowerCase()),
            size: (a, b) => {
                if (a.collapsible !== b.collapsible) {
                    return a.collapsible ? -1 : 1; // Directories first
                }
            ...
            modified: (a, b) => {
                ...
                if (a.collapsible)
                    return 0; // Both directories
                const dateA = ((_a = a.lastModified) === null || _a === void 0 ? void 0 : _a.getTime()) || 0;
                const dateB = ((_b = b.lastModified) === null || _b === void 0 ? void 0 : _b.getTime()) || 0;
                return dateA - dateB;
            }
        };
        const comparator = comparators[sortBy] || comparators.type;
    ...
    getChildren(element) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const response = yield this.makeRequestWithCache(apiUrl);
                ...
                const items = response.data.content.map((item) => new FileItem(item.name, item.type === 'directory', item.path, item));
    ...
    openFile(filePath) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (extension === 'ipynb') {
                    yield this.openJupyterNotebook(filePath);
                    return;
                }
    ...
    openJupyterNotebook(filePath) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const uri = vscode.Uri.parse(`jupyter-remote:/${filePath}`);
                // Try to open as notebook first
                try {
                    yield vscode.commands.executeCommand('vscode.openWith', uri, 'jupyter-notebook');
                }
                catch (notebookError) {
                    ...
                    const document = yield vscode.workspace.openTextDocument(uri);
                    yield vscode.languages.setTextDocumentLanguage(document, 'json');
                    yield vscode.window.showTextDocument(document);
                }
            }
            catch (error) {
    ...
    saveFileToJupyter(filePath, content) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.isConnected || !this.axiosInstance) {
                vscode.window.showErrorMessage('Not connected to Jupyter Server.');
                return;
            ...
            try {
                const apiUrl = `api/contents/${filePath}`;
                yield this.axiosInstance.put(apiUrl, {
                    content,
                    type: 'file',
                    format: 'text'
                });
    ...
    createDirectory(uri) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield this.axiosInstance.put(apiUrl, { type: 'directory', content: null });
    ...
    readFile(uri) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const content = yield this.fetchFileContent(uri.path.slice(1));
    ...
    writeFile(uri, content, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const path = uri.path.startsWith('/') ? uri.path.substring(1) : uri.path;
            yield this.saveFileToJupyter(path, content.toString());
            const parentUri = vscode.Uri.parse(`jupyter-remote:${this.extractParentPath(uri.path)}`);
    ...
    delete(uri, options) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const stat = yield Promise.resolve(statResult);
                if (stat.type === vscode.FileType.Directory && options.recursive) {
                    ...
                    yield this.deleteDirectoryRecursive(itemPath);
                }
                else {
                    ...
                    const response = yield this.axiosInstance.delete(apiUrl);
                ...
                if ((_e = (_d = error.response) === null || _d === void 0 ? void 0 : _d.data) === null || _e === void 0 ? void 0 : _e.message) {
    ...
    deleteDirectoryRecursive(dirPath) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const response = yield this.axiosInstance.get(listUrl);
                const contents = response.data.content || [];
                console.log(`Directory ${dirPath} contains ${contents.length} items:`, contents.map((item) => `${item.name} (${item.type})`));
                ...
                if (contents.length === 0) {
                    ...
                    yield this.axiosInstance.delete(deleteUrl);
                ...
                for (const item of contents) {
                    ...
                    if (item.type === 'directory') {
                        ...
                        yield this.deleteFileWithRetry(itemPath);
                    }
                }
                ...
                try {
                    ...
                    console.error(`Failed to delete directory ${dirPath}:`, ((_a = dirError.response) === null || _a === void 0 ? void 0 : _a.data) || dirError.message);
                    // If it still says "not empty", let's check what's left
                    if ((_d = (_c = (_b = dirError.response) === null || _b === void 0 ? void 0 : _b.data) === null || _c === void 0 ? void 0 : _c.message) === null || _d === void 0 ? void 0 : _d.includes('not empty')) {
                        console.log(`Directory still reported as not empty, checking contents again...`);
                        try {
                            const checkResponse = yield this.axiosInstance.get(listUrl);
                            const remainingContents = checkResponse.data.content || [];
                            console.log(`Remaining contents in ${dirPath}:`, remainingContents.map((item) => `${item.name} (${item.type})`));
                            // Try to delete remaining items
                            for (const item of remainingContents) {
                                console.log(`Retrying deletion of: ${item.path}`);
                                try {
                                    if (item.type === 'directory') {
                                        yield this.deleteDirectoryRecursive(item.path);
                                    }
                                    else {
                                        yield this.axiosInstance.delete(`api/contents/${item.path}`);
                                    }
                                }
                                catch (retryError) {
                ...
                console.error(`Failed to delete directory ${dirPath}:`, ((_e = error.response) === null || _e === void 0 ? void 0 : _e.data) || error.message);
                throw error;
            }
        });
    }
    deleteFileWithRetry(itemPath, maxRetries = 3, delay = 1000) {
        var _a, _b;
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.axiosInstance) {
                throw new Error('Not connected to Jupyter Server');
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    yield this.axiosInstance.delete(fileDeleteUrl);
                    console.log(`Successfully deleted file: ${itemPath} (attempt ${attempt})`);
                    return;
                }
                catch (fileError) {
                    console.error(`Failed to delete file ${itemPath} (attempt ${attempt}):`, ((_a = fileError.response) === null || _a === void 0 ? void 0 : _a.data) || fileError.message);
                    if (attempt === maxRetries) {
                        // On final attempt, provide more informative error message
                        const extension = (_b = itemPath.split('.').pop()) === null || _b === void 0 ? void 0 : _b.toLowerCase();
    ...
    forceDelete(uri) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                try {
                    yield this.delete(uri, { recursive: true });
                ...
                yield this.forceDeleteRecursive(itemPath);
    ...
    forceDeleteRecursive(dirPath, maxAttempts = 5) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.axiosInstance) {
                throw new Error('Not connected to Jupyter Server');
            }
            ...
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                try {
                    ...
                    if (contents.length === 0) {
                        ...
                        yield this.axiosInstance.delete(`api/contents/${dirPath}`);
                        console.log(`Successfully deleted empty directory: ${dirPath}`);
                        return;
                    }
                    // Delete all items with extended retry and ignore individual failures
                    const deletePromises = contents.map((item) => __awaiter(this, void 0, void 0, function* () {
                        try {
                            if (item.type === 'directory') {
                                yield this.forceDeleteRecursive(item.path, 3); // Fewer attempts for subdirectories
                            }
                            else {
                                yield this.deleteFileWithRetry(item.path, 5, 500); // More attempts, shorter delay
                            }
                        }
                        catch (error) {
                    ...
                    yield Promise.all(deletePromises.map((p) => p.catch(() => { })));
    ...
    rename(oldUri, newUri, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield this.axiosInstance.patch(apiUrl, { path: newPath });
    ...
    newFile(item) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const fileName = yield vscode.window.showInputBox({ prompt: 'Enter file name', ignoreFocusOut: true });
            if (fileName) {
                const newFilePath = `${parentPath}/${fileName}`.replace('//', '/');
                const uri = vscode.Uri.parse(`jupyter-remote:${newFilePath}`);
                yield this.writeFile(uri, new Uint8Array(Buffer.from('')), { create: true, overwrite: false });
    ...
    newFolder(item) {
        return __awaiter(this, void 0, void 0, function* () {
            const parentPath = item ? (item.collapsible ? item.uri : this.extractParentPath(item.uri)) : this.remotePath;
            const folderName = yield vscode.window.showInputBox({ prompt: 'Enter folder name', ignoreFocusOut: true });
            if (folderName) {
                const newFolderPath = `${parentPath}/${folderName}`.replace('//', '/');
                const uri = vscode.Uri.parse(`jupyter-remote:${newFolderPath}`);
                yield this.createDirectory(uri);
                this.invalidateCacheForPath(parentPath);
                this.refresh();
            }
        });
    ...
    renameFile(item) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const newName = yield vscode.window.showInputBox({ prompt: 'Enter new name', value: oldName, ignoreFocusOut: true });
            if (newName && newName !== oldName) {
                ...
                yield this.rename(oldUri, newUri, { overwrite: false });
                this.refresh();
            }
        });
    }
    deleteFile(item) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield vscode.window.showWarningMessage(`Are you sure you want to delete ${item.label}?`, { modal: true }, 'Delete', 'Force Delete');
            if (result === 'Delete') {
                ...
                const uri = vscode.Uri.parse(`jupyter-remote:${item.uri}`);
                yield this.forceDelete(uri);
                this.invalidateCacheForPath(item.uri);
                this.refresh();
            }
        });
    }
    forceDeleteFile(item) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield vscode.window.showWarningMessage(`Force delete will attempt aggressive deletion of ${item.label}. This may take longer and cannot be undone. Continue?`, { modal: true }, 'Force Delete');
    ...
    newFolderInRoot() {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.newFolder();
        });
    }
    processFileOperation(files, targetPath, operation, operationFn) {
        return __awaiter(this, void 0, void 0, function* () {
            let successCount = 0;
            let errorCount = 0;
            ...
            for (const fileUri of files) {
                try {
                    yield operationFn(fileUri.fsPath, targetPath);
                    successCount++;
                }
                catch (error) {
    ...
    uploadFile(targetDirectory) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const fileUri of fileUris) {
                try {
                    yield this.uploadSingleFile(fileUri.fsPath, parentPath);
                    successCount++;
                }
                catch (error) {
    ...
    uploadFolder(targetDirectory) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const parentPath = targetDirectory ? targetDirectory.uri : this.remotePath;
            const folderUris = yield vscode.window.showOpenDialog({
            ...
            for (const folderUri of folderUris) {
                try {
                    ...
                    yield this.uploadFolderRecursive(folderUri.fsPath, targetPath);
                    successCount++;
                }
                catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (response.data.type === 'file') {
                    ...
                    yield fs.promises.writeFile(saveUri.fsPath, fileContent);
    ...
    handleDrop(target, dataTransfer, token) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log('handleDrop called with target:', target === null || target === void 0 ? void 0 : target.label);
            // Check for internal tree item moves
            const internalTransfer = dataTransfer.get('application/vnd.code.tree.jupyterFileExplorer');
            if (internalTransfer) {
                yield this.handleInternalMove(internalTransfer, target);
                return;
            }
            // Handle external file drops
            const uriListTransfer = dataTransfer.get('text/uri-list');
            if (uriListTransfer) {
                yield this.handleExternalFileUpload(uriListTransfer, target);
                return;
            }
            vscode.window.showInformationMessage('No valid files detected in drop operation.');
        });
    }
    handleExternalFileUpload(uriListTransfer, target) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const uriListData = yield uriListTransfer.asString();
                ...
                for (const uri of uris) {
                    if (uri.scheme === 'file') {
                        try {
                            const stat = yield fs.promises.stat(uri.fsPath);
                            if (stat.isFile()) {
                                yield this.uploadSingleFile(uri.fsPath, targetPath);
                                successCount++;
                            }
                            else if (stat.isDirectory()) {
                                const folderName = path.basename(uri.fsPath);
                                yield this.uploadFolderRecursive(uri.fsPath, `${targetPath}/${folderName}`.replace('//', '/'));
                                successCount++;
                            }
                        }
                        catch (error) {
    ...
    handleInternalMove(transferItem, target) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                console.log('handleInternalMove called');
                const sourceItems = transferItem.value;
                ...
                for (const sourceItem of sourceItems) {
                    try {
                        ...
                        yield this.moveItem(sourceItem.uri, newPath);
                        successCount++;
                    }
                    catch (error) {
    ...
    moveItem(sourcePath, targetPath) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const response = yield this.axiosInstance.patch(apiUrl, {
                ...
                if (error.response) {
                    ...
                    console.error('Request data:', (_b = error.config) === null || _b === void 0 ? void 0 : _b.data);
                    ...
                    if (error.response.status === 404) {
                        ...
                        throw new Error(`Move failed: ${error.response.status} - ${((_c = error.response.data) === null || _c === void 0 ? void 0 : _c.message) || 'Unknown server error'}`);
                    }
                }
                else {
    ...
    uploadSingleFile(localFilePath, remotePath) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const fileContent = yield fs.promises.readFile(localFilePath);
                ...
                const response = yield this.axiosInstance.put(apiUrl, requestData);
                ...
                throw new Error(`Upload failed for ${path.basename(localFilePath)}: ${((_g = (_f = error.response) === null || _f === void 0 ? void 0 : _f.data) === null || _g === void 0 ? void 0 : _g.message) || error.message}`);
            }
        });
    }
    uploadFolderRecursive(localFolderPath, remotePath) {
        var _a, _b, _c, _d, _e, _f, _g;
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.axiosInstance) {
                throw new Error('Not connected to Jupyter Server');
            ...
            try {
                ...
                const dirResponse = yield this.axiosInstance.put(apiUrl, {
                    type: 'directory'
                });
                console.log(`Directory created successfully:`, dirResponse.status);
                // Read directory contents
                const entries = yield fs.promises.readdir(localFolderPath, { withFileTypes: true });
                ...
                for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                        yield this.uploadFolderRecursive(localEntryPath, remoteEntryPath);
                    }
                    else if (entry.isFile()) {
                        yield this.uploadSingleFile(localEntryPath, remotePath);
                    }
                }
            }
            catch (error) {
                ...
                throw new Error(`Folder upload failed for ${path.basename(localFolderPath)}: ${((_g = (_f = error.response) === null || _f === void 0 ? void 0 : _f.data) === null || _g === void 0 ? void 0 : _g.message) || error.message}`);
            }
        });
    }
...
class FileItem extends vscode.TreeItem {
    constructor(label, collapsible, uri, fileInfo) {
...
exports.JupyterContentProvider = JupyterContentProvider;
